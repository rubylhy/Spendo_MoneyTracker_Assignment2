...
// useReducer is better than useState here because all fields are
// related and often reset or loaded together as one unit.
// This avoids 5 separate useState calls and setter functions.
// ---------------------------------------------------------------
const formReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'RESET':
      // Reset all fields at once after saving or cancelling
...
const Dashboard = ({ token, onLogout }) => {
  ...
  // --- Simple independent states — useState is appropriate here ---
  ...
  const [error, setError]                       = useState(null);  // stores API connection error

  // --- useReducer for form — 5 related fields that change together ---
  const [formData, dispatch] = useReducer(formReducer, {
    title: '', category: 'Food', amount: '', date: '', description: ''
  });

  // --- useRef for search bar — lets us focus it without re-rendering ---
  // useRef is used here instead of useState because focusing a DOM element
  // doesn't need to trigger a re-render
  const searchRef = useRef(null);

  // Category colours map — defined once and reused throughout component
  ...
  // Fetch expenses when token changes — only fetch if logged in (Slide 16)
  useEffect(() => { if (token) fetchExpenses(); }, [token]);

  // Auto-focus search bar when dashboard loads — useRef gives direct DOM access
  useEffect(() => {
    if (searchRef.current) searchRef.current.focus();
  }, []); // empty array = runs once on mount only

  // ---------------------------------------------------------------
  // authFetch — reusable fetch helper
  // Automatically adds Authorization header to every request (Slide 18)
  // Avoids repeating the header in every fetch call
  // ---------------------------------------------------------------
  const authFetch = async (url, options = {}) => {
    const response = await fetch(`${API_URL}${url}`, {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${token}`, // attach JWT token — Slide 18
        ...options.headers,
      }
    });
  ...
  // ---------------------------------------------------------------
  // FETCH — load all expenses for the logged-in user
  // ---------------------------------------------------------------
  const fetchExpenses = async () => {
    setLoading(true);
    setError(null); // clear previous error before retrying
    try {
  ...
  // Dispatch UPDATE_FIELD action when any form input changes
  ...
  // ---------------------------------------------------------------
  // SUBMIT — create a new expense or update an existing one
  // ---------------------------------------------------------------
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (submitting) return; // prevent double submit if user clicks twice

    // Input validation — specific message for each missing field
    ...
    setSubmitting(true); // disable button to prevent double submit
    try {
      ...
      setSubmitting(false); // re-enable button regardless of outcome
    }
  };

  // Reset form fields and exit edit mode — dispatches RESET action
  const resetForm = () => {
    setEditingId(null);
    dispatch({ type: 'RESET' }); // resets all 5 fields at once
  };

  // Load an expense into the form for editing — dispatches LOAD action
  const startEdit = (exp) => {
    setEditingId(exp.id);
    dispatch({
      type: 'LOAD',
  ...
  const deleteExpense = async (id) => {
    ...
    try {
      ...
      if (!response) return;
      // Update local state directly — avoids an extra fetch round-trip
  ...
  // ---------------------------------------------------------------
  // DATA CALCULATIONS — derived from expenses state
  // ---------------------------------------------------------------

  // Filter expenses for the selected month only
  const monthlyExpenses = expenses.filter(exp => exp.date.startsWith(selectedMonth));

  // Sum all amounts for the selected month
  const totalAmount = monthlyExpenses.reduce((sum, item) => sum + Number(item.amount), 0);

  // Apply all active filters: search term, category, and month
  ...
  // Total spent per category for the selected month (used in breakdown chart)
  const categoryTotals = monthlyExpenses.reduce((acc, exp) => {
    acc[exp.category] = (acc[exp.category] || 0) + Number(exp.amount);
    return acc;
  }, {});

  // Build last 6 months of spending data for the trend chart
  const getTrendData = () => {
    const months = [];
    const today  = new Date();
    for (let i = 5; i >= 0; i--) {
      // Setting day to 1 avoids issues with months having different lengths (e.g. Feb)
      const d     = new Date(today.getFullYear(), today.getMonth() - i, 1);
      const year  = d.getFullYear();
      // padStart ensures format is always "YYYY-MM" not "YYYY-M"
  ...
  // Used to calculate bar heights as percentage of the tallest bar
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-80 bg-white border-r p-6 overflow-y-auto shrink-0 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {/* Submit button — disabled and greyed out while submitting */}
            <button
              type="submit"
              disabled={submitting}
        ...
        <section className="flex-1 p-6 overflow-y-auto bg-gray-50 border-r">
          <div className="flex justify-between items-center mb-6">
            ...
            {/* Search bar — useRef attached here for auto-focus on mount */}
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-[10px] font-black whitespace-nowrap transition-all
                  ${selectedCategory === cat
                    ...
                    : 'bg-white text-gray-400 border hover:border-indigo-200'}`}
              >
                {cat.toUpperCase()}
              </button>
            ))}
          ...
          <div className="space-y-3">
            ...
            {/* Error state — shown when API fails to connect, with retry button */}
            {error && !loading && (
            ...
            {/* Empty state — connected but no expenses match the filters */}
        ...
        <aside className="w-72 bg-white p-6 overflow-y-auto shrink-0 shadow-inner">
          ...
          {/* Bar chart — height of each bar is proportional to maxTrend */}
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-2 group relative h-full justify-end">
                  ...
                  <span className={`text-[8px] font-bold uppercase truncate w-full text-center
                    ...
                    {/* Using "-02" avoids timezone issues when parsing month strings */}
          ...
          {/* Category breakdown — shows percentage of total for each category */}
